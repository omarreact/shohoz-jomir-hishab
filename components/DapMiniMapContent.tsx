...
import { MapContainer, TileLayer, Polygon, useMap, Tooltip, LayersControl } from "react-leaflet";
...
export default function DapMiniMapContent({ plotData }: DapMiniMapContentProps) {
  ...
  useEffect(() => {
    ...
    if (plotData?.geometry?.rings && plotData.geometry.rings.length > 0) {
      ...
      const coords = ring.map((point: number[]) => [point[1], point[0]] as [number, number]);
  ...
  const isMS = plotData?.plot_type_custom === "MS দাগ" || plotData?.plot_type === "ms_plot_no";
    ...
    <div style={{ height: "100%", width: "100%", minHeight: "300px" }}>
      ...
      >
        ...
        <Polygon 
          ...
          pathOptions={{ color: 'red', fillColor: '#ff0000', fillOpacity: 0.2, weight: 3 }} 
        >
          <Tooltip direction="center" permanent className="bg-transparent border-0 shadow-none text-danger fw-bolder fs-5 label-text-shadow">
            {plotLabel}
          </Tooltip>
        </Polygon>
