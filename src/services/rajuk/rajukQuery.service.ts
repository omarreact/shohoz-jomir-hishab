...
async function requestLayer<T>(layerId: number, params: Record<string, string | number | boolean | undefined>) {
  const token = await getValidToken(`${RAJUK_DB}/${layerId}`);
  ...
  if (response.status === 499) throw new Error("RAJUK token required or expired");
...
export async function getPlots(filters: RajukPlotFilters): Promise<RajukPlotCollection> {
  ...
  return requestLayer<RajukPlotCollection>(0, { where: clauses.length ? clauses.join(" AND ") : "1=0", outFields: "objectid,plot_no,p_guid,rs_plot_no,address_search,Shape__Area,Shape__Length", returnGeometry: true, outSR: 4326, resultRecordCount: Math.min(Math.max(filters.resultRecordCount ?? 50, 1), 100), resultOffset: Math.max(filters.resultOffset ?? 0, 0), orderByFields: "plot_no ASC" });
}

export async function identifyByPoint(lat: number, lng: number): Promise<RajukIdentifyResult> {
  return requestLayer<RajukIdentifyResult>(0, { geometry: `${lng},${lat}`, geometryType: "esriGeometryPoint", spatialRel: "esriSpatialRelIntersects", inSR: 4326, outSR: 4326, outFields: "objectid,plot_no,p_guid,rs_plot_no,address_search,Shape__Area,Shape__Length", returnGeometry: true, resultRecordCount: 5 });
}
