...
const attr = (a: Record<string, unknown>, names: string[]) => {
  for (const n of names) if (str(a[n])) return str(a[n]);
  return "";
};
...
async function requestLayer<T>(layerId: number, params: Record<string, string | number | boolean | undefined>): Promise<T> {
  ...
  const authenticatedQuery = new URLSearchParams(query); authenticatedQuery.set("token", token);
  ...
  if (isAuthError(response.status, data)) {
    await invalidateToken(RAJUK_SERVER); const freshToken = await refreshToken(RAJUK_SERVER); authenticatedQuery.set("token", freshToken);
    response = await fetchWithRetry(`${serverUrl}/query?${authenticatedQuery.toString()}`, { timeoutMs: 20_000, retries: 1 }); data = await parseJson<T & ArcGisEnvelope>(response);
  }
