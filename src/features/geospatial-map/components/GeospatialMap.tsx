...
type BasemapKey = "osm" | "light" | "satellite";
...
const BASEMAPS: Record<BasemapKey, { label: string; url: string; attribution: string }> = {
  osm: { label: "OpenStreetMap", url: "https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", attribution: "© OpenStreetMap contributors" },
  light: { label: "Light", url: "https://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}{r}.png", attribution: "© OpenStreetMap © CARTO" },
  satellite: { label: "Satellite", url: "https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}", attribution: "© Esri" },
};
...
export default function GeospatialMap() {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      const base = L.tileLayer(BASEMAPS.light.url, { attribution: BASEMAPS.light.attribution, maxZoom: 21, crossOrigin: true }).addTo(map);
          ...
          {tab === "layers" && <>
            ...
            {LAYERS.map((layer) => <div className={styles.layerCard} key={layer.key}>
              <div className={styles.layerRow}>
                ...
                <div className={styles.layerInfo}><div className={styles.layerName}>{layer.label}</div><div className={styles.layerMeta}>{layer.description}</div></div>
                <label className={styles.toggle}><input type="checkbox" checked={layers[layer.key]} onChange={(e) => setLayers((current) => ({ ...current, [layer.key]: e.target.checked }))}/><span className={styles.toggleTrack}/></label>
              </div>
              <div className={styles.opacityRow}><span>অপাসিটি</span><input type="range" min="0" max="1" step=".05" value={opacity[layer.key]} onChange={(e) => setOpacity((current) => ({ ...current, [layer.key]: Number(e.target.value) }))}/><span>{Math.round(opacity[layer.key] * 100)}%</span></div>
            </div>)}
          </>}
          {tab === "basemap" && <>
            <div className={styles.sectionTitle}>বেসম্যাপ নির্বাচন</div>
            <div className={styles.basemapGrid}>{(Object.entries(BASEMAPS) as [BasemapKey, typeof BASEMAPS[BasemapKey]][]).map(([key, value]) => <button type="button" key={key} className={`${styles.baseButton} ${basemap === key ? styles.baseActive : ""}`} onClick={() => setBasemap(key)}><span className={styles.baseIcon}>{key === "satellite" ? "◈" : key === "light" ? "□" : "⊕"}</span>{value.label}</button>)}</div>
            <div className={styles.sectionTitle}>ইন্টার‌্যাকশন</div>
            <button type="button" className={`${styles.baseButton} ${identifyMode ? styles.baseActive : ""}`} onClick={() => setIdentifyMode((value) => !value)}><MousePointer2 size={17}/><br/>{identifyMode ? "Identify চালু আছে" : "প্লটে ক্লিক করে Identify"}</button>
          </>}
          {tab === "results" && <>
