...
import { toBanglaDigits } from "@/src/shared/i18n/bangla";
import {
  buildMsPlotSearchParams,
  filterExactPlotMatches,
  normalizePlotInput,
  plotNo,
  type PlotMode,
} from "./plotSearch";
...
function bn(value: string | number): string {
...
function SelectField({
  ...
  return (
    <div className="relative min-w-0">
      ...
      {hint ? <p className="mt-0.5 text-[11px] leading-4 text-[var(--muted-foreground)]">{hint}</p> : null}
...
function useAddressCascade(kind: PlotMode) {
  ...
  useEffect(() => {
    ...
    void fetch(`/api/rajuk/query?action=districts&kind=${kind}`, { cache: "no-store", signal: controller.signal })
      .then(async (response) => {
        const data = await response.json();
  ...
  const selectedDistrict = districts.find((item) => item.d_guid === dGuid);
  const selectedUpazila = upazilas.find((item) => item.t_guid === tGuid);
  const selectedMouza = mouzas.find((item) => item.m_guid === mauzaId);
...
export default function RajukTestPageClient() {
  ...
  }, [loc.dGuid, loc.tGuid, loc.mauzaId, mode]);
  ...
  const runSearch = useCallback(async () => {
    ...
    if (mode === "ms") {
      const query = buildMsPlotSearchParams(
        plotInput,
        {
          district: loc.selectedDistrict?.m_district,
          upazila: loc.selectedUpazila?.upazila_ps,
          mouza: loc.selectedMouza?.mauza,
          jl: loc.selectedMouza?.jl_no,
        },
        200,
      );
      const url = `/api/rajuk/query?${query.toString()}`;
      setLastRequestUrl(url);
      setLoadingPlots(true);
      try {
        const data = await apiJson(url);
        const features = filterExactPlotMatches(
          (data.features ?? []) as RajukPlotFeature[],
          bare,
          "ms",
        );
        if (!features.length) {
          const place = [loc.selectedMouza?.mauza, loc.selectedUpazila?.upazila_ps]
            .filter(Boolean)
            .join(" · ");
          setSearchError(
            place
              ? `${place} এলাকায় এম এস প্লট ${bn(bare)} পাওয়া যায়নি। অবস্থান ফিল্টার কমিয়ে আবার চেষ্টা করুন।`
              : `এম এস প্লট ${bn(bare)} পাওয়া যায়নি।`,
          );
          return;
        }
    ...
    if (!match) {
      setSearchError(`আর এস প্লট ${bn(bare)} পাওয়া যায়নি।`);
      return;
    }
    setSelected(match);
  ...
  }, [plotInput, mode, plotsLoaded, plots, loc]);
  ...
  const resultRows = useMemo(() => {
    ...
    const prefix = mode === "rs" ? "আরএস" : "এমএস";
    return [
      { label: `${prefix} প্লট নম্বর`, value: bn(rawNo || bare) },
      { label: "প্লট নং", value: bn(bare) },
      { label: "জে.এল. নং", value: bn(attrStr(attributes, ["jl_no", "rs_jl_no", "ms_jl_no"])) },
      { label: "পরিমাণ (শতাংশ)", value: area.isValid ? `${bn(formatAreaValue(area.shotok, 4))} শতাংশ` : "—" },
      { label: "পরিমাণ (কাঠা)", value: area.isValid ? `${bn(formatAreaValue(area.katha))} কাঠা` : "—" },
  ...
  const msRows = useMemo(
    () => msInside.map((feature, index) => {
      ...
      return {
        no: bn(no),
        shotok: area.isValid ? bn(formatAreaValue(area.shotok, 4)) : "—",
        katha: area.isValid ? bn(formatAreaValue(area.katha)) : "—",
      };
    }),
    [msInside],
  );
      ...
      <div className="mx-auto max-w-4xl space-y-5">
        <header className="space-y-3">
          ...
          <p className="max-w-2xl text-sm leading-6 text-[var(--muted-foreground)]">
            আর এস ও এম এস প্লট নম্বরকে জেলা, উপজেলা, মৌজা এবং জে.এল. তথ্য দিয়ে আরও নির্দিষ্টভাবে অনুসন্ধান করুন।
          </p>
        ...
        <section className="rounded-2xl border border-[var(--border-color)] bg-[var(--card-bg)] p-4 shadow-sm sm:p-5 md:p-6">
          <div className="mb-4 flex items-start gap-3">
            <div className="mt-0.5 grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-emerald-50 text-[#17663A] dark:bg-emerald-950/30">
              <Filter size={18} />
            </div>
            <div>
              <h2 className="text-base font-bold">
                {mode === "rs" ? "ঠিকানা নির্বাচন" : "নির্দিষ্ট এম এস প্লটের এলাকা নির্বাচন"}
              </h2>
              <p className="mt-1 text-xs leading-5 text-[var(--muted-foreground)]">
                {mode === "rs"
                  ? "আর এস প্লট লোড করতে জেলা, উপজেলা এবং মৌজা/জে.এল. নির্বাচন আবশ্যক।"
                  : "এগুলো ঐচ্ছিক। একই এম এস প্লট নম্বর একাধিক স্থানে থাকলে জেলা → উপজেলা → মৌজা/জে.এল. নির্বাচন করলে ফলাফল অনেক বেশি নির্দিষ্ট হবে।"}
              </p>
            </div>
          </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <SelectField
              label="জেলা"
              hint={mode === "ms" ? "প্রথমে জেলা নির্বাচন করুন" : undefined}
              value={loc.dGuid}
            ...
            <SelectField
              label="থানা / উপজেলা"
              hint={mode === "ms" ? "জেলার ভিতরের নির্দিষ্ট উপজেলা/সার্কেল" : undefined}
              value={loc.tGuid}
            ...
            <div className="sm:col-span-2">
              <SelectField
                label="মৌজা + জে.এল."
                hint={mode === "ms" ? "MS layer-এর address_search মিলিয়ে server-side ফলাফল সংকুচিত হবে" : undefined}
                value={loc.mauzaId}
              ...
              >
                ...
                {loc.mouzas.map((mouza) => (
                  <option key={mouza.m_guid} value={mouza.m_guid}>
                    {mouza.mauza} — জে.এল. {bn(mouza.jl_no)}
                  </option>
                ))}
              </SelectField>
            </div>
          ...
          {mode === "rs" ? (
            ...
            <div className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50/70 px-3.5 py-3 text-xs leading-5 text-emerald-950 dark:border-emerald-900/60 dark:bg-emerald-950/20 dark:text-emerald-100">
              <strong>কীভাবে কাজ করে:</strong> এম এস প্লট লেয়ারে আলাদা প্রশাসনিক GUID না থাকলেও LandBD রাজউকের shared জেলা/উপজেলা/মৌজা তালিকা ব্যবহার করে এবং নির্বাচিত উপজেলা, মৌজা ও জে.এল. শব্দগুলো এম এস <span translate="no">address_search</span>-এর সঙ্গে মিলিয়ে ফলাফল সীমিত করে। মূল এম এস API ও layer অপরিবর্তিত থাকে।
            </div>
          )}
        </section>
        ...
        <section className="rounded-2xl border border-[var(--border-color)] bg-[var(--card-bg)] p-4 shadow-sm sm:p-5 md:p-6">
          ...
          <p className="mt-1 text-xs leading-5 text-[var(--muted-foreground)]">
            {mode === "rs"
              ? plotsLoaded
                ? `${bn(plots.length)}টি আর এস প্লট লোড হয়েছে।`
                : "আগে নির্বাচিত মৌজার আর এস প্লট লোড করুন।"
              : loc.selectedMouza
                ? `${loc.selectedMouza.mauza} · জে.এল. ${bn(loc.selectedMouza.jl_no)}-এর মধ্যে এম এস প্লট নম্বর খোঁজা হবে।`
                : loc.selectedUpazila
                  ? `${loc.selectedUpazila.upazila_ps}-এর মধ্যে এম এস প্লট নম্বর খোঁজা হবে।`
                  : "শুধু প্লট নম্বর দিয়েও খোঁজা যাবে; নির্দিষ্ট ফলাফলের জন্য উপরের অবস্থান ফিল্টার ব্যবহার করুন।"}
          </p>
          <div className="mt-4 flex flex-col gap-2 sm:flex-row">
            <input
              ...
              placeholder={mode === "rs" ? "যেমন: ৪৬৭ বা আরএস-৪৬৭" : "যেমন: ৪৭১১ বা এমএস-৪৭১১"}
        ...
        {mode === "ms" && msMatches.length > 1 ? (
          <section className="rounded-2xl border border-[var(--border-color)] bg-[var(--card-bg)] p-4 shadow-sm sm:p-5">
            <h2 className="text-base font-bold">একই নম্বরের এম এস ফলাফল — {bn(msMatches.length)}টি</h2>
            <p className="mt-1 text-xs text-[var(--muted-foreground)]">মৌজা, জে.এল. ও ঠিকানা দেখে সঠিক প্লটটি নির্বাচন করুন।</p>
                ...
                return (
                  ...
                  >
                    ...
                    <span className="mt-1 block text-xs text-[var(--muted-foreground)]">
                      {area.isValid ? `${bn(formatAreaValue(area.shotok, 4))} শতাংশ · ${bn(formatAreaValue(area.katha))} কাঠা` : "জমির পরিমাণ পাওয়া যায়নি"}
                    </span>
                  </button>
                );
              })}
          ...
          <section className="space-y-4 rounded-2xl border border-[var(--border-color)] bg-[var(--card-bg)] p-4 shadow-sm sm:p-5 md:p-6">
              ...
              {mode === "rs" ? (
                <div className="mt-4">
                  <h3 className="mb-2 text-sm font-bold">
                    ...
                    {loadingMsInside ? " (লোড হচ্ছে…)" : msRows.length ? ` — ${bn(msRows.length)}টি` : " — পাওয়া যায়নি"}
                  </h3>
                  {msRows.length ? (
                        ...
                        <tbody>
                          {msRows.map((row, index) => (
                            <tr key={`${row.no}-${index}`} className="border-t border-violet-100">
                              <td className="px-3 py-2">{bn(index + 1)}</td>
            ...
            {isLoggedIn && lastRequestUrl ? (
              <div className="rounded-xl border border-[var(--border-color)] bg-[var(--secondary)] p-3">
                ...
                <code data-bangla-ignore="true" className="block break-all rounded-lg bg-[var(--card-bg)] px-3 py-2 font-mono text-xs">GET {lastRequestUrl}</code>
              </div>
            ) : null}
