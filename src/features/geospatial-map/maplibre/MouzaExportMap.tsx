...
export default function MouzaExportMap() {
  ...
  const [downloadFormat, setDownloadFormat] = useState<"geotiff" | "raw">("geotiff");
  const [downloadLayers, setDownloadLayers] = useState<"rs" | "ms" | "combined">("combined");
  ...
  const handleGisDownload = useCallback(async () => {
    if (!loadedMouza || isExporting) return;
    setIsExporting(true);
    setError("");
    try {
      setExportPhase("মৌজা সীমানা প্রস্তুত…");
      setExportPhase("ArcGIS টাইল লোড…");
      const params = new URLSearchParams({
        mouza: loadedMouza,
      ...
      if (!response.ok) {
        ...
        throw new Error(payload.error || "GIS ডাউনলোড ব্যর্থ");
      }
      setExportPhase("ফাইল প্রস্তুত…");
      const blob = await response.blob();
      const width = response.headers.get("X-LandBD-Width");
      const height = response.headers.get("X-LandBD-Height");
      const zoom = response.headers.get("X-LandBD-Zoom");
      const res = response.headers.get("X-LandBD-Resolution");
      const cd = response.headers.get("Content-Disposition") || "";
      const match = /filename="?([^";]+)"?/i.exec(cd);
      const filename =
        match?.[1] ||
        `landbd-${loadedMouza.replace(/[^\p{L}\p{N}]+/gu, "-")}-${downloadFormat === "raw" ? "raw.zip" : "geotiff.tif"}`;
      ...
      setStatus(
        `${loadedMouza}: ${downloadFormat === "geotiff" ? "GeoTIFF" : "RAW"} · ${width || "?"}×${height || "?"}px · z${zoom || "?"} · ${res ? Number(res).toFixed(3) + "m" : ""}`,
      );
      setExportPhase("");
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : "GIS ডাউনলোড ব্যর্থ");
      setExportPhase("");
    } finally {
      setIsExporting(false);
    }
  }, [downloadFormat, downloadLayers, isExporting, loadedMouza]);
  ...
  const handlePreviewPng = useCallback(async () => {
    ...
    setError("");
    try {
      setExportPhase("প্রিভিউ PNG…");
      await exportMouzaClippedPng(map, {
        width: 3840,
        height: 2160,
        clipRings,
        filename: `landbd-${loadedMouza.replace(/[^\p{L}\p{N}]+/gu, "-")}-preview.png`,
      });
      setStatus(`${loadedMouza}: প্রিভিউ PNG ডাউনলোড`);
      setExportPhase("");
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : "প্রিভিউ ব্যর্থ");
      setExportPhase("");
    } finally {
      setIsExporting(false);
    }
  ...
  return (
    <div className="relative h-full min-h-[min(70vh,720px)] w-full overflow-hidden bg-[var(--background)] md:min-h-[min(75vh,900px)]">
      <div
        ref={containerRef}
      ...
      <div className="pointer-events-none absolute inset-x-0 top-0 z-10 flex items-start justify-between gap-3 p-3 sm:p-4">
        ...
        {loadedMouza ? (
          <div className="pointer-events-auto w-[min(96vw,320px)] space-y-2 rounded-2xl border border-white/60 bg-white/95 p-3 shadow-xl backdrop-blur dark:border-slate-700/70 dark:bg-slate-950/95">
            <p className="text-[11px] font-semibold text-slate-600 dark:text-slate-300">
              GIS-গ্রেড ডাউনলোড · শুধু {loadedMouza}
            </p>
            <p className="text-[10px] leading-snug text-amber-700 dark:text-amber-300">
              উচ্চ রেজোলিউশন GeoTIFF/RAW — ফাইল বড় হতে পারে। স্ক্রিনশট নয়।
            </p>
            <div className="grid grid-cols-2 gap-2">
              <label className="text-[10px] font-semibold text-slate-500">
                ...
                <select
                  ...
                  onChange={(e) => setDownloadFormat(e.target.value as "geotiff" | "raw")}
                  className="mt-0.5 w-full rounded-lg border border-slate-300 bg-white px-2 py-1.5 text-xs dark:border-slate-600 dark:bg-slate-900 dark:text-white"
                >
                  <option value="geotiff">GeoTIFF</option>
                  <option value="raw">RAW (ZIP)</option>
                </select>
              </label>
              <label className="text-[10px] font-semibold text-slate-500">
                লেয়ার
                <select
                  value={downloadLayers}
                  onChange={(e) => setDownloadLayers(e.target.value as "rs" | "ms" | "combined")}
                  className="mt-0.5 w-full rounded-lg border border-slate-300 bg-white px-2 py-1.5 text-xs dark:border-slate-600 dark:bg-slate-900 dark:text-white"
                >
            ...
            <button
              type="button"
              onClick={() => void handleGisDownload()}
              disabled={isExporting}
              className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-slate-900 px-3 py-2 text-sm font-semibold text-white transition hover:bg-slate-700 disabled:cursor-wait disabled:opacity-70 dark:bg-white dark:text-slate-900"
            >
              {isExporting ? <Loader2 className="h-4 w-4 animate-spin" /> : <Download className="h-4 w-4" />}
              {isExporting ? exportPhase || "জেনারেট হচ্ছে…" : downloadFormat === "geotiff" ? "GeoTIFF ডাউনলোড" : "RAW ডাউনলোড"}
            </button>
            <button
              type="button"
              onClick={() => void handlePreviewPng()}
              disabled={isExporting}
              className="inline-flex w-full items-center justify-center gap-2 rounded-xl border border-slate-300 px-3 py-1.5 text-[11px] font-semibold text-slate-700 hover:bg-slate-50 disabled:opacity-60 dark:border-slate-600 dark:text-slate-200 dark:hover:bg-slate-800"
            >
              প্রিভিউ PNG (ঐচ্ছিক)
            </button>
          </div>
      ...
      <div className="pointer-events-none absolute bottom-20 left-1/2 z-10 w-[min(92vw,520px)] -translate-x-1/2 space-y-2 md:bottom-4">
        {error ? (
      ...
      {selected ? (
        <div className="absolute bottom-20 right-3 z-20 w-[min(92vw,340px)] rounded-2xl border border-slate-200 bg-white/95 p-4 shadow-2xl backdrop-blur md:bottom-4 dark:border-slate-700 dark:bg-slate-950/95">
          <div className="mb-3 flex items-center justify-between">
