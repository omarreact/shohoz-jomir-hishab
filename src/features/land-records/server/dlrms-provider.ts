...
export const dlrmsLandRecordProvider: LandRecordProvider = {
  ...
  async getKhatian(surveyKey, id, signal): Promise<KhatianDetails> {
    ...
    let base = {
      ...
      DIVISION_NAME: optionalString(row, "DIVISION_NAME"),
      DISTRICT_NAME: optionalString(row, "DISTRICT_NAME"),
      UPAZILA_NAME: optionalString(row, "UPAZILA_NAME"),
      JL_NUMBER: optionalString(row, "JL_NUMBER"),
      MOUZA_NAME: optionalString(row, "MOUZA_NAME"),
      SURVEY_ID: optionalNumber(row, "SURVEY_ID"),
      SURVEY_NAME: optionalString(row, "SURVEY_NAME"),
      TOTAL_LAND: optionalString(row, "TOTAL_LAND", "totalLand", "LAND_AMOUNT", "landAmount"),
