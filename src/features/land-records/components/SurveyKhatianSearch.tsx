...
import { exportKhatianPng } from "../lib/khatian-png-export";
...
export default function SurveyKhatianSearch() {
  ...
  const downloadKhatianPng = async () => {
    ...
    try {
      ...
      const result = await exportKhatianPng({
      ...
      console.error("Khatian PNG export failed", error);
      setLocalError("PNG তৈরি করা যায়নি। আবার চেষ্টা করুন।");
    } finally {
      ...
      <main className="mx-auto w-full max-w-7xl min-w-0 overflow-x-hidden px-3 py-6 sm:px-6 sm:py-8">
        ...
        {viewMode === "details" ? (
          <section id="khatian-details-panel" className="space-y-4 print:mt-0">
            <div className="flex flex-col gap-2 print:hidden sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
              <button
                type="button"
                onClick={backToSearch}
                className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-lg border border-[var(--border-color)] bg-[var(--card-bg)] px-4 text-sm font-semibold hover:bg-[var(--secondary)] sm:w-auto"
              >
                <ArrowLeft size={16} />
              ...
              {selectedKhatian ? (
                <button
                  ...
                  onClick={() => void downloadKhatianPng()}
                  disabled={downloadingImage || loading.khatian}
                  className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-lg border border-[var(--border-color)] px-4 text-sm font-semibold hover:bg-[var(--secondary)] disabled:opacity-50 sm:w-auto"
                >
                  {downloadingImage ? <Loader2 size={16} className="animate-spin" /> : <Download size={16} />}
                  {downloadingImage ? "PNG তৈরি হচ্ছে…" : "উচ্চ রেজোলিউশন PNG ডাউনলোড"}
                </button>
              ) : null}
            </div>
