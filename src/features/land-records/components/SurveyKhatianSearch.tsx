...
export default function SurveyKhatianSearch() {
  ...
  const downloadKhatianPng = async () => {
    ...
    try {
      ...
      // High pixel density for sharp Bangla text & table borders (lossless PNG).
      ...
      const canvas = await html2canvas(node, {
        ...
        ignoreElements: (el) => {
          ...
          // Tech/debug blocks and any explicitly marked UI chrome
          if (el.dataset.excludeExport === "1") return true;
          return false;
        },
        onclone: (_document, cloned) => {
          // Force archival light theme so dark-mode text stays readable on white PNG
          cloned.style.backgroundColor = "#ffffff";
          cloned.style.color = "#0f172a";
          cloned.classList.remove("dark");
          const root = cloned.closest("html") ?? _document.documentElement;
      ...
      link.download = `khatian-${safeNo}.png`;
      link.href = dataUrl;
      link.click();
    } catch (error) {
