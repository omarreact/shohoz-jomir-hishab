...
export default function FaraezResult({ results, exportRef, religion }: Props) {
  ...
  return <div id="resultSection" className="container mx-auto pb-8 animate-in fade-in zoom-in-95 mt-4">
    <Card className="shadow-lg overflow-hidden border-success/30 border-2">
      ...
      <CardContent ref={exportRef as React.RefObject<HTMLDivElement>} className="relative isolate overflow-hidden bg-white p-4 text-slate-900 md:p-8 rounded-none">
      ...
      <CardFooter className="bg-muted/30 p-6 no-print flex flex-wrap justify-center gap-4 border-t border-success/30 rounded-b-xl">
        ...
        <Button onClick={downloadFaraezCsv} variant="outline" className="px-6 font-bold shadow-sm rounded-full border-success text-success hover:bg-success hover:text-success-foreground"><FileSpreadsheet size={18} className="mr-2" /> CSV</Button>
