...
function ringCentroid(rings: number[][][]): { lat: number; lng: number } | null {
  const ring = rings?.[0]; if (!ring?.length) return null;
  let sumLng = 0, sumLat = 0, n = 0;
  for (const pt of ring) { if (pt.length < 2) continue; sumLng += pt[0]; sumLat += pt[1]; n += 1; }
  return n ? { lng: sumLng / n, lat: sumLat / n } : null;
}
function googleMapsUrl(feature: RajukPlotFeature): string | null {
  const c = ringCentroid(feature.geometry.rings); if (!c) return null;
  const label = encodeURIComponent(String(feature.attributes.rs_plot_no || feature.attributes.ms_plot_no || feature.attributes.plot_no || "RAJUK plot"));
...
export default function PlotMap({ feature, features = [] }: { feature: RajukPlotFeature; features?: RajukPlotFeature[] }) {
  const mapRef = useRef<HTMLDivElement>(null); const leafletRef = useRef<any>(null); const layersRef = useRef<Record<string, any>>({});
  const [base, setBase] = useState<BaseLayer>("satellite"); const [showRs, setShowRs] = useState(true); const [showMs, setShowMs] = useState(true); const [ready, setReady] = useState(false);
  const allFeatures = useMemo(() => features.length ? features : [feature], [features, feature]);
  const mapsLink = useMemo(() => googleMapsUrl(feature), [feature]);

  useEffect(() => {
    if (!mapRef.current || leafletRef.current) return; let disposed = false;
    const init = async () => {
      const L = (await import("leaflet")).default; await import("leaflet/dist/leaflet.css"); if (disposed || !mapRef.current) return;
      const map = L.map(mapRef.current, { center: [23.8103, 90.4125], zoom: 16, minZoom: 8, maxZoom: 21, zoomControl: true }); leafletRef.current = map;
      const street = L.tileLayer("https://{s}.basemaps.cartocdn.com/rastertiles/voyager/{z}/{x}/{y}{r}.png", { attribution: "&copy; OSM &copy; CARTO", subdomains: "abcd", maxZoom: 21 });
      ...
      const rsOverlay = L.tileLayer("/api/rajuk/tile/rs/{z}/{y}/{x}", { attribution: "&copy; RAJUK RS", maxZoom: 21, opacity: 0.75, errorTileUrl: "data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7" });
      const msOverlay = L.tileLayer("/api/rajuk/tile/ms/{z}/{y}/{x}", { attribution: "&copy; RAJUK MS", maxZoom: 21, opacity: 0.75, errorTileUrl: "data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7" });
      const boundaries = L.featureGroup().addTo(map); const selectedLayer = L.geoJSON(null, { style: { color: "#006a4e", weight: 4, fillColor: "#22c55e", fillOpacity: 0.28 } }).addTo(map);
      layersRef.current = { street, satellite, rsOverlay, msOverlay, boundaries, selectedLayer }; satellite.addTo(map); rsOverlay.addTo(map); msOverlay.addTo(map); setReady(true);
    };
    void init(); return () => { disposed = true; if (leafletRef.current) { leafletRef.current.remove(); leafletRef.current = null; layersRef.current = {}; } };
  }, []);

  useEffect(() => { const map = leafletRef.current; const { street, satellite } = layersRef.current; if (!map || !street || !satellite) return; if (base === "satellite") { map.removeLayer(street); if (!map.hasLayer(satellite)) satellite.addTo(map); } else { map.removeLayer(satellite); if (!map.hasLayer(street)) street.addTo(map); } }, [base, ready]);
  useEffect(() => { const map = leafletRef.current; const { rsOverlay, msOverlay } = layersRef.current; if (!map || !rsOverlay || !msOverlay) return; if (showRs) { if (!map.hasLayer(rsOverlay)) rsOverlay.addTo(map); } else if (map.hasLayer(rsOverlay)) map.removeLayer(rsOverlay); if (showMs) { if (!map.hasLayer(msOverlay)) msOverlay.addTo(map); } else if (map.hasLayer(msOverlay)) map.removeLayer(msOverlay); }, [showRs, showMs, ready]);
  ...
  useEffect(() => {
    ...
    import("leaflet").then(({ default: L }) => {
      ...
      if (selectedBounds.isValid()) map.fitBounds(selectedBounds, { padding: [36, 36], maxZoom: 19 }); else if (allBounds.isValid()) map.fitBounds(allBounds, { padding: [36, 36], maxZoom: 18 });
    });
