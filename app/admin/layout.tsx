...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[var(--background)] text-[var(--foreground)]">
      ...
      <aside
        className={`fixed inset-y-3 left-3 z-50 flex flex-col overflow-hidden rounded-2xl border border-[var(--border-color)] bg-[var(--card-bg)] shadow-md transition-all duration-200 ${
          isSidebarOpen ? "w-64 translate-x-0" : "-translate-x-[110%] lg:w-20 lg:translate-x-0"
        }`}
      >
        <div className="flex h-16 shrink-0 items-center justify-between border-b border-[var(--border-color)] px-4">
          ...
          >
            ...
            <span className={`font-bold tracking-tight ${!isSidebarOpen ? "lg:hidden" : ""}`}>
              LandBD Admin
            </span>
          </Link>
          <button
            onClick={() => setIsSidebarOpen(false)}
            className="rounded-lg p-1.5 text-[var(--muted-foreground)] hover:bg-[var(--secondary)] lg:hidden"
          >
            <X size={18} />
          </button>
        </div>
        ...
        <nav className="flex-1 space-y-1 overflow-y-auto p-3">
          <div
            className={`px-3 pb-2 pt-1 text-[11px] font-semibold uppercase tracking-wider text-[var(--muted-foreground)] ${
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                href={item.path}
                key={item.path}
                className={`group flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium no-underline transition-colors ${
                  active
        ...
        <div className="border-t border-[var(--border-color)] p-3">
          <button
            ...
            className={`flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-red-600 transition-colors hover:bg-red-50 dark:hover:bg-red-950/20 ${
      ...
      <div
        className={`min-h-screen transition-all duration-200 ${
          isSidebarOpen ? "lg:pl-[292px]" : "lg:pl-24"
        }`}
      >
          ...
          <div className="flex h-14 items-center justify-between rounded-2xl border border-[var(--border-color)] bg-[color-mix(in_srgb,var(--card-bg)_90%,transparent)] px-3 shadow-sm backdrop-blur-xl sm:px-4">
            <div className="flex items-center gap-3">
              <button
                ...
                className="flex h-9 w-9 items-center justify-center rounded-xl text-[var(--muted-foreground)] hover:bg-[var(--secondary)]"
              >
                <Menu size={19} />
              </button>
              <div>
                <h1 className="text-sm font-semibold sm:text-base">{currentTitle}</h1>
            ...
            <div className="flex items-center gap-2">
              {isMounted && (
                <button
                  ...
                  className="flex h-9 w-9 items-center justify-center rounded-xl border border-[var(--border-color)] hover:bg-[var(--secondary)]"
                >
                  {theme === "dark" ? <Moon size={16} /> : <Sun size={16} />}
                </button>
              )}
              <Link href="/" className="hidden no-underline sm:block">
                <Button variant="outline" size="sm">
                  <Globe size={15} /> ওয়েবসাইট
                </Button>
              </Link>
              <div className="hidden items-center gap-2 border-l border-[var(--border-color)] pl-3 md:flex">
                <div className="flex h-8 w-8 items-center justify-center rounded-full bg-[var(--secondary)] text-xs font-bold">
                  {userName.charAt(0).toUpperCase()}
                </div>
                <div>
                  <div className="text-xs font-semibold">{userName || "Admin"}</div>
                  <div className="text-[10px] text-[var(--muted-foreground)]">{userRole}</div>
                </div>
              </div>
            </div>
          </div>
