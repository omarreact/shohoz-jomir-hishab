...
export async function PUT(
  ...
  try {
    ...
    // 1. Update Auth Profile if name provided
    if (validated.name) {
...
export async function DELETE(
  ...
  try {
    ...
    // Soft-delete Firestore Profile to preserve comments/history
    await docRef.update({
      status: "deleted",
      updatedAt: new Date().toISOString(),
    });
