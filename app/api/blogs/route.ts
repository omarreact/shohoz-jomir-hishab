...
export async function GET(req: NextRequest) {
  try {
    ...
    // Fallback in-memory sort to avoid requiring composite indexes on every filtered query
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!existingSnapshot.empty) {
      return NextResponse.json({ success: false, message: "A blog with this title already exists" }, { status: 409 });
    }
