...
import { exportMouzaPublicationPdf } from "@/src/services/rajuk/mouzaPublicationPdf.service";
...
async function runExport(input: unknown) {
  ...
  if (parsed.data.format === "vector-pdf") {
    return { result: await exportMouzaPublicationPdf({
      mouza: parsed.data.mouza, jl: parsed.data.jl, layers: parsed.data.layers, satellite: parsed.data.satellite,
    }) };
  }
