...
function toUiResults(result: FaraezDomainResult, assets: AssetsInput): HeirResult[] {
  ...
  return result.allocations.map((allocation) => {
    ...
    const measurement = result.measurementAllocations[measurementIndex];
...
export default function FaraezPage() {
    ...
    try {
      if (religion === "muslim") {
        ...
        const input: FaraezInput = {
          religion: "muslim", deceasedGender: gender, heirs,
          estate: { land: assets.land, gold: assets.gold, cash: assets.cash, funeralCost: assets.funeralCost, debt: assets.debt, wasiyat: assets.wasiyat },
  ...
  const downloadMultiPagePDF = async () => {
    ...
    try {
      ...
      await html2pdf().set({ margin: [15, 10, 15, 10] as [number, number, number, number], filename: "Faraez_Result.pdf", image: { type: "jpeg" as const, quality: 0.98 }, html2canvas: { scale: 2, useCORS: true, windowWidth: 800 }, jsPDF: { unit: "mm", format: "a4", orientation: "portrait" as const }, pagebreak: { mode: ["avoid-all", "css", "legacy"] } }).from(element).save();
  ...
  const downloadExcel = () => {
    ...
    results.filter((r) => r.count > 0).forEach((res) => {
      for (let i = 1; i <= res.count; i++) {
        ...
        const measurementText = m ? `${m.ana}A ${m.gonda}G ${m.kora}K ${m.kranti}Kr ${m.til}T` : "";
        csvContent += `${heirName},${fractionText},${measurementText},${land},${gold},${cash},${res.reasoning.replace(/,/g, " ")}\n`;
      }
    });
    const blob = new Blob(["\uFEFF" + csvContent], { type: "text/csv;charset=utf-8;" });
    const url = URL.createObjectURL(blob); const link = document.createElement("a"); link.href = url; link.download = "Faraez_Result.csv"; document.body.appendChild(link); link.click(); document.body.removeChild(link); URL.revokeObjectURL(url);
  };
